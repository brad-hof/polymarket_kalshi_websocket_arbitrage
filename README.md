# polymarket_kalshi_websocket_arbitrage
Find arbitrage opportunities using Kalshi and Polymarket websocket API's.

1. Use the 'get_poly_clob_token_ID.py' script to fetch the CLOB token for a specific market (Lakers vs Celtics - Lakers) given your category of choice (nba, mlb, NFL, etc).
2. Go to the Kalshi website and find the market ticker for the same market but on the other side (Lakers vs Celtics - Lakers).
